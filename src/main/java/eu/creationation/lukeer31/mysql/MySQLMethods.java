...
public class MySQLMethods {
	...
	public static Map<Integer, String> getStoredPlayers() {
		...
		try {
			...
			while (rs.next()) {
				...
				StoredPlayers.put(rs.getInt("player_id"),
	...
	public static Map<String, String> loadBannedPlayers() {
		// Load all the banned players from the database and insert into a
		// hashmap
		// Get the DB
		java.sql.Statement st = Config.st;
		// Create the hashmap
		...
		try {
			// Run the query
			ResultSet result = st
					.executeQuery("SELECT * FROM `cn_bans` WHERE `ban_expired`=`0`");
			// Loop through the resultset and add to the hasmap
			while (result.next()) {
				// Push into the hashmap
				BannedPlayers.put(
						GlobalMethods.getPlayerUsername(result.getInt(1)),
						result.getString(4));
			}
		} catch (SQLException ex) {
	...
	public static Map<String, String> loadMutedPlayers() {
		...
		try {
			// Run the query
			ResultSet result = st
					.executeQuery("SELECT * FROM `cn_mutes` WHERE `mute_expired`=`0`");
			// Loop through the resultset and add to the hasmap
			while (result.next()) {
				// Push into the hashmap
				MutedPlayers.put(
