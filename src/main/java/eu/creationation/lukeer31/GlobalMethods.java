...
public class GlobalMethods {
	...
	public static ArrayList<Player> getPlayerCSV(String players, boolean onlineonly){
		//Split the CSV
		String[] playerlist = players.split(",");
		ArrayList<Player> playerlistout = new ArrayList<Player>();		
		if(playerlist.length == 0){
			//There is only one player specified. Just get that one and return it
			...
			Player player = plugin.getServer().getPlayer(players);
				...
				if(player == null){
					...
					player = (Player) plugin.getServer().getOfflinePlayer(players);
				}
				playerlistout.add(player);
				return playerlistout;
			}
		...
		for(int i = 0; i < playerlist.length; i++){			
			...
			if(onlineonly == true){
				...
				//Now get the player
				if(!username.equals("") & plugin.getServer().getPlayer(username) != null){
				...
				if(!username.equals("") & plugin.getServer().getPlayer(username) != null){
					...
					//The player is offline, get em and add them to the arraylist
					playerlistout.add((Player) plugin.getServer().getOfflinePlayer(username));	
				}			
			}
		}
